...
export default function CartItem({ item }) {
  const price = parseFloat(item.price || 0); // Corrected
  const quantity = item.quantity || 1;
  const total = price * quantity;

  return (
    <div className="bg-white rounded-lg p-5 shadow-md flex flex-col md:flex-row justify-between items-center gap-4">
      ...
      <div className="flex items-center gap-5 w-full md:w-2/3">
        ...
        <div className="flex-1">
          ...
          <button className="text-sm text-[#B92123] mt-2 hover:underline">
            Remove item
          </button>
        </div>
      </div>
      ...
      <div className="text-right w-full md:w-1/3">
        ...
        <button className="bg-[#B92123] text-white text-xs md:text-sm py-2 px-4 rounded-lg hover:bg-red-600 transition w-full">
          Checkout
        </button>
      </div>
    </div>
